...
export default function NotificationsTable() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      setLoading(true);
      try {
        const response = await getUserNotifications({ limit: 100, unread_only: false });
        setData(response);
      } catch (err) {
  ...
  const columns = [
    ...
    columnHelper.accessor('is_read', {
      ...
      cell: (info) => <Text color={textColor} fontSize="sm" fontWeight="700">{info.getValue() ? 'Yes' : 'No'}</Text>,
    }),
