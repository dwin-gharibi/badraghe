...
import { getUserReports, deleteReport } from 'services/api';
...
export default function ReportsTable() {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        ...
        if (searchTerm) params.status_filter = searchTerm;
        ...
        toast({
          title: 'Error',
          description: err.response?.data?.detail || err.message || 'Failed to fetch reports',
          status: 'error',
          duration: 5000,
          isClosable: true,
        });
  ...
  const handleDelete = async (reportId) => {
    try {
      ...
      toast({
        title: 'Error',
        description: err.response?.data?.detail || err.message || 'Failed to delete report',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
  ...
  const columns = [
    ...
    columnHelper.accessor('category', {
      id: 'category',
      header: () => <Text justifyContent="space-between" align="center" fontSize={{ sm: '10px', lg: '12px' }} color="gray.400">CATEGORY</Text>,
      cell: (info) => <Text color={textColor} fontSize="sm" fontWeight="700">{info.getValue()}</Text>,
    }),
    columnHelper.accessor('status', {
      ...
      cell: (info) => (
        <Badge
          colorScheme={info.getValue() === 'resolved' ? 'green' : 'orange'}
          variant="solid"
        >
          {info.getValue()}
        </Badge>
      ...
      cell: (info) => (
        <Stack direction="row" spacing={2}>
          <IconButton
            ...
            colorScheme="blue"
            aria-label="View"
            onClick={() => console.log('View report', info.row.original.id)}
          />
          <IconButton
            icon={<FaEdit />}
            colorScheme="yellow"
            aria-label="Edit"
            onClick={() => console.log('Edit report', info.row.original.id)}
          />
          <IconButton
            icon={<FaTrash />}
            colorScheme="red"
