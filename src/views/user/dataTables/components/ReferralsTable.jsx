...
export default function ReferralsTable() {
  ...
  useEffect(() => {
    const fetchReferrals = async () => {
      ...
      try {
        ...
        if (searchTerm) params.referrer_id = searchTerm;
        const response = await listReferrals(params);
        setData(response.data || response);
        setTotalCount(response.total || response.length);
      } catch (err) {
        console.error('Full Error:', err);
        setError(err.response?.data?.detail || err.message || 'Failed to fetch referrals');
        toast({
          title: 'Error',
          description: err.response?.data?.detail || err.message || 'Failed to fetch referrals',
          status: 'error',
          duration: 5000,
          isClosable: true,
        });
  ...
  const handleDelete = async (referralId) => {
    try {
      ...
      toast({
        title: 'Error',
        description: err.response?.data?.detail || err.message || 'Failed to delete referral',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
  ...
  const columns = [
    ...
    columnHelper.accessor('referrer_id', {
      id: 'referrer_id',
      header: () => <Text justifyContent="space-between" align="center" fontSize={{ sm: '10px', lg: '12px' }} color="gray.400">REFERRER ID</Text>,
      cell: (info) => <Text color={textColor} fontSize="sm" fontWeight="700">{info.getValue()}</Text>,
    }),
    columnHelper.accessor('referred_id', {
      id: 'referred_id',
      header: () => <Text justifyContent="space-between" align="center" fontSize={{ sm: '10px', lg: '12px' }} color="gray.400">REFERRED ID</Text>,
      cell: (info) => <Text color={textColor} fontSize="sm" fontWeight="700">{info.getValue()}</Text>,
    }),
      ...
      cell: (info) => (
        <Stack direction="row" spacing={2}>
          <IconButton
            ...
            colorScheme="blue"
            aria-label="View"
            onClick={() => console.log('View referral', info.row.original.id)}
          />
          <IconButton
            icon={<FaEdit />}
            colorScheme="yellow"
            aria-label="Edit"
            onClick={() => console.log('Edit referral', info.row.original.id)}
          />
          <IconButton
            icon={<FaTrash />}
            colorScheme="red"
